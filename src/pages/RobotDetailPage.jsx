    ...
    <div>
      <div className="page-header">
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
          ...
          <button className="btn sm" onClick={() => can('pdf_export') ? setShowPDFModal(true) : (window.__onManageLicense && window.__onManageLicense())}
            title={can('pdf_export') ? 'Exportar PDF' : 'Disponível no plano Pro'}
            style={{ display:'flex', alignItems:'center', gap:4, opacity: can('pdf_export') ? 1 : 0.5 }}>
          ...
          <button className="btn sm primary" onClick={handleSaveSettings} disabled={savingSettings}>
            {savingSettings ? '...' : 'Salvar'}
          </button>
        </div>
      </div>
      ...
      {showPDFModal && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.45)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center' }}
          ...
          <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:'var(--radius-lg)', padding:28, minWidth:340, boxShadow:'0 8px 32px rgba(0,0,0,.18)' }}
            onClick={e => e.stopPropagation()}>
            <div style={{ fontWeight:700, fontSize:16, marginBottom:6 }}>Exportar PDF</div>
            <div style={{ fontSize:12, color:'var(--text-muted)', marginBottom:18 }}>Selecione as seções a incluir no relatório</div>
            {[
              { key:'visaoGeral', label:'Visão Geral', desc:'Métricas, Monte Carlo e pontuação' },
              { key:'graficos', label:'Gráficos', desc:'Curva de capital, distribuição, mensal' },
              { key:'operacoes', label:'Operações', desc:'Lista das últimas 100 operações' },
              { key:'contaReal', label:'Conta Real', desc:'Comparativo backtest × real' },
              { key:'validacao', label:'Validação', desc:'Critérios, especialistas e observação' },
              { key:'periodos', label:'Períodos', desc:'IS, OOS e Paper Trading' },
            ].map(({ key, label, desc }) => (
              <div key={key} onClick={() => setPDFSections(p => ({ ...p, [key]: !p[key] }))}
                style={{ display:'flex', alignItems:'center', gap:12, padding:'10px 12px', marginBottom:6, borderRadius:'var(--radius)', cursor:'pointer',
                  background: pdfSections[key] ? 'var(--accent-bg)' : 'var(--bg)', border:`1px solid ${pdfSections[key] ? 'var(--accent)' : 'var(--border)'}` }}>
                <div style={{ width:18, height:18, borderRadius:4, border:`2px solid ${pdfSections[key] ? 'var(--accent)' : 'var(--border)'}`,
                  background: pdfSections[key] ? 'var(--accent)' : 'transparent', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                  {pdfSections[key] && <svg width="10" height="10" viewBox="0 0 10 10"><path d="M2 5l2.5 2.5L8 3" stroke="white" strokeWidth="1.5" strokeLinecap="round" fill="none"/></svg>}
                </div>
                <div>
                  <div style={{ fontWeight:600, fontSize:13, color: pdfSections[key] ? 'var(--accent)' : 'var(--text)' }}>{label}</div>
            ...
            <div style={{ display:'flex', gap:8, marginTop:20 }}>
              <button className="btn primary" style={{ flex:1 }} onClick={() => {
                exportRobotPDF(robot, metrics, adjOps, periods, pdfSections)
                setShowPDFModal(false)
              }}>
                📄 Exportar PDF
              </button>
              <button className="btn" onClick={() => setShowPDFModal(false)}>Cancelar</button>
            </div>
          </div>
        </div>
      )}
