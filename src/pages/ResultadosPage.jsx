...
export default function ResultadosPage() {
  ...
  async function loadAll() {
    ...
    try {
      ...
      for(const p of (pList||[])){
        ...
        opsMap[p.id]=names.flatMap(n=>opsByName[n]||[]).sort((a,b)=>opSortKey(b.abertura).localeCompare(opSortKey(a.abertura)))
      }
      setAllOps(opsMap)
      const sMap={}; strats.forEach(s=>sMap[s]=opsByName[s]||[])
      setStratOps(sMap)
      const unused = null
    } catch(e){console.error('[MentoradosPage]',e)}
  ...
  return(
    <div style={{padding:'24px 28px',maxWidth:1200,margin:'0 auto'}}>
      ...
      {apiError&&<div style={{padding:'20px 24px',background:'rgba(240,96,96,0.08)',border:'1px solid rgba(240,96,96,0.25)',borderRadius:10}}><div style={{fontWeight:700,color:'var(--danger)',marginBottom:8}}>⚠ API não encontrada (window.api.mport)</div><div style={{fontSize:13,color:'var(--text-muted)',marginBottom:12}}>Substitua <strong>main.js</strong> e <strong>preload.js</strong> e reinicie.</div><button className="btn sm primary" onClick={loadAll}>Tentar novamente</button></div>}
